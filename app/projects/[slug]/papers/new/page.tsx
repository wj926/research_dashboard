...
export default async function NewPaperPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const action = createPaper.bind(null, slug);

  return (
    <div className="max-w-3xl">
      <Link
        href={`/projects/${slug}/papers`}
        className="inline-flex items-center gap-1 text-sm text-accent-fg hover:underline mb-4"
      >
        <ArrowLeftIcon size={14} /> Back to papers
      </Link>
      <h1 className="text-lg font-semibold mb-1">New paper in {project.name}</h1>
      <p className="text-sm text-fg-muted mb-4">{project.description}</p>
      <form action={action} className="space-y-4 bg-white border border-border-default rounded-md p-6">
        <div>
